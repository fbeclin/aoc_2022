...
def compare(left: list, right: list, depth: int = 0):
    ...
    while i < len(left) and i < len(right):
        if isinstance(left[i], int) and isinstance(right[i], int):
            ...
            if left[i] > right[i]:
                ...
                print(
                    f"{left_padding}- Right side is smaller, so inputs are NOT in the right order"
                )
                return False
            elif left[i] < right[i]:
                left_padding = " " * (depth + 2)
                print(
                    f"{left_padding}- Left side is smaller, so inputs are in the right order"
                )
                return True
            ...
            if isinstance(left[i], int):
                print(
                    f"{left_padding}- Mixed types; convert left to [{left[i]}] and retry comparison"
                )
                return compare([left[i]], right[i], depth + 1)
            else:
                print(
                    f"{left_padding}- Mixed types; convert right to [{right[i]}] and retry comparison"
                )
                return compare(left[i], [right[i]], depth + 1)
        i += 1
    ...
    if i != len(left) and i >= len(right):
        left_padding = " " * (depth + 1)
        print(
            f"{left_padding}- Right side ran out of items, so inputs are NOT in the right order"
        )
        return False
    left_padding = " " * (depth + 1)
    print(
        f"{left_padding}- Left side ran out of items, so inputs are in the right order"
    )
    return True
